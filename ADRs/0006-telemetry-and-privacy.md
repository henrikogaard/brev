...
| Google OAuth token exchange | Authorization code, PKCE verifier, and client ID exchanged with `oauth2.googleapis.com`; refresh-token exchanges thereafter | During user-initiated Google account setup, user-initiated Google calendar/contacts feature enablement (ADR-0072), and on token refresh | Off until the user adds a Google account or enables a Google PIM feature | Account add/remove; Settings → Calendar & Contacts Google feature enablement |
