...
| Google Picker file chooser | The account's OAuth access token, the app's Google API key and app ID handed to the Google-hosted picker page (`apis.google.com/js/api.js`); file selection happens inside Google's UI | Only when the user opens a Drive attach/save sheet on a Gmail API account after the `drive.file` grant | **Off** | Drive attach/save action; revoke the Google grant |
| Google Drive attachment calls | Google OAuth access token plus `files.get`/`files.list` metadata lookups, `files.get?alt=media` byte downloads, `files.export` conversions of picked Workspace files, and multipart `files.create`/`files.update` uploads to `www.googleapis.com`/`googleapis.com` | Only when the user attaches a picked Drive file to a draft or saves a message attachment to a picked Drive folder, after the `drive.file` opt-in | **Off** | Drive attach/save action; revoke the Google grant |
