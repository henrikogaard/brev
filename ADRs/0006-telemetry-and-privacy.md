...
| Keep Offline body prefetch | Existing IMAP account credentials or Gmail OAuth token plus mailbox/message identifiers and a message-body fetch request to the configured provider, when needed by the backend | Only when the user turns Keep Offline on in a message list, Unified Inbox, reader or conversation menu; cache-first and best-effort | **Off** | Explicit per-message Keep Offline toggle; removing the local pin does not fetch or immediately delete cached content |
...
| Sparkle update check | Appcast request metadata such as app version, release ring, IP address, and user agent; the appcast and release notes are fetched from `henrikogaard.github.io` and the DMG from `github.com` / `objects.githubusercontent.com` | Once per launch in configured direct-download macOS releases of either ring (Stable or Nightly) | On for direct-download macOS only | Settings → Updates |
...
| Google OAuth token exchange | Authorization code, PKCE verifier, and client ID exchanged with `oauth2.googleapis.com`; refresh-token exchanges thereafter | During user-initiated Google account setup, user-initiated Google calendar/contacts/tasks feature enablement (ADR-0072), the user-initiated Google Drive attachment opt-in (#14), and on token refresh | Off until the user adds a Google account, enables a Google PIM feature, or uses a Drive action | Account add/remove; Settings → Calendar & Contacts Google feature enablement; Drive opt-in prompt |
| Google OpenID UserInfo verification | The short-lived Google access token in a bearer header; the response is used only to read the verified email and stable subject | Immediately after a user-initiated Google authorization-code exchange, including feature-triggered reauthorization | Off until the user adds a Google account or enables a Google PIM feature | Account add/remove; Settings → Calendar & Contacts Google feature enablement |
...
| CalDAV/CardDAV source setup | Unauthenticated RFC 6764 well-known GET for domain discovery, then the user-entered credential (bearer token or app password) plus a `PROPFIND current-user-principal` validation request to the discovered or manually entered endpoint | Only when the user connects or reconnects a calendar/contacts source in Settings | **Off** | Settings → Calendar & Contacts source setup/removal |
| CalDAV/CardDAV collection discovery | The source credential plus `PROPFIND` requests for the principal home-set and a Depth:1 collection listing (names, colors, permissions, sync hints) | Only when the user connects a source or chooses Refresh Collections on it | **Off** | Settings → Calendar & Contacts source list |
| Google Calendar/Contacts/Tasks collection listing | Google OAuth access token plus `calendarList` / `contactGroups` / `tasklists.list` read requests to `www.googleapis.com` / `people.googleapis.com` / `tasks.googleapis.com` | Only when the user enables the Google calendar/contacts/tasks feature or chooses Refresh Collections on the source | **Off** | Settings → Calendar & Contacts source list |
| CalDAV event sync | The source credential plus `REPORT` requests (`sync-collection`, or `calendar-query` + `calendar-multiget` on servers without sync tokens) to the collection URLs the source exposed | Only when the user chooses Sync Now on a calendar source or enables its sync | **Off** | Settings → Calendar & Contacts source list |
| Google Calendar event sync | Google OAuth access token plus `events.list` read requests (paged, with sync tokens for incremental passes) to `www.googleapis.com` | Only when the user chooses Sync Now on a Google calendar source or enables its sync | **Off** | Settings → Calendar & Contacts source list |
| CardDAV contact sync | The source credential plus `REPORT` requests (`sync-collection`, or `addressbook-query` + `addressbook-multiget` on servers without sync tokens) to the address book URLs the source exposed | Only when the user chooses Sync Now on a contacts source or enables its sync | **Off** | Settings → Calendar & Contacts source list |
| Google People contact sync | Google OAuth access token plus paged `people.connections.list` read requests (with sync tokens for incremental passes) to `people.googleapis.com` | Only when the user chooses Sync Now on a Google contacts source or enables its sync | **Off** | Settings → Calendar & Contacts source list |
| CalDAV task sync | The source credential plus `REPORT` requests (`sync-collection`, or a VTODO-filtered `calendar-query` + `calendar-multiget` on servers without sync tokens) to the task collection URLs the source exposed | Only when the user chooses Sync Now on a tasks source or enables its sync | **Off** | Settings → Calendar & Contacts source list |
| Google Tasks sync | Google OAuth access token plus paged `tasks.list` read requests (with `updatedMin` incremental passes) to `tasks.googleapis.com` | Only when the user chooses Sync Now on a Google tasks source or enables its sync | **Off** | Settings → Calendar & Contacts source list |
| CalDAV event write | The source credential plus `PUT`/`DELETE` requests carrying the event payload to the collection URLs the source exposed; `If-Match`/`If-None-Match` preconditions ride every mutation | Only when the user enables Editing on a calendar source and then creates, edits, or deletes an event | **Off** | Settings → Calendar & Contacts source list |
| Google Calendar event write | Google OAuth access token plus `events.insert`/`patch`/`delete` requests carrying the event payload to `www.googleapis.com`; the cached etag rides as `If-Match` | Only when the user enables Editing on a Google calendar source (which first re-authorizes the `calendar.events` scope) and then creates, edits, or deletes an event | **Off** | Settings → Calendar & Contacts source list |
| CalDAV task write | The source credential plus `PUT`/`DELETE` requests carrying the VTODO payload to the task collection URLs the source exposed; `If-Match`/`If-None-Match` preconditions ride every mutation | Only when the user enables Editing on a tasks source and then creates, edits, completes, moves, or deletes a task | **Off** | Settings → Calendar & Contacts source list |
| Google Tasks write | Google OAuth access token plus `tasks.insert`/`patch`/`delete`/`move` requests carrying the task payload to `tasks.googleapis.com`; the cached etag rides as `If-Match` | Only when the user enables Editing on a Google tasks source (which first re-authorizes the `tasks` scope) and then creates, edits, completes, moves, or deletes a task | **Off** | Settings → Calendar & Contacts source list |
| CardDAV contact write | The source credential plus `PUT`/`DELETE` requests carrying the vCard payload — including inline `PHOTO` bytes when the user sets a photo — to the address book URLs the source exposed; `If-Match`/`If-None-Match` preconditions ride every mutation and edits merge into the stored vCard so fields Brev does not read are preserved | Only when the user enables Editing on a contacts source and then creates, edits, or deletes a contact | **Off** | Settings → Calendar & Contacts source list |
| Google People contact write | Google OAuth access token plus `people:createContact`/`updateContact`/`deleteContact` requests carrying the masked contact fields, and `people:updateContactPhoto`/`deleteContactPhoto` requests carrying only the photo bytes, to `people.googleapis.com`; the cached etag rides as a precondition | Only when the user enables Editing on a Google contacts source (which first re-authorizes the `contacts` scope) and then creates, edits, sets/removes a photo on, or deletes a contact | **Off** | Settings → Calendar & Contacts source list |
| Google Picker file chooser | The account's OAuth access token, the app's Google API key and app ID handed to the Google-hosted picker page (`apis.google.com/js/api.js`); file selection happens inside Google's UI | Only when the user opens a Drive attach/save sheet — from compose, the reader, or the event editor — on a Gmail API account after the `drive.file` grant | **Off** | Drive attach/save action; revoke the Google grant |
| Google Drive attachment calls | Google OAuth access token plus `files.get`/`files.list` metadata lookups, `files.get?alt=media` byte downloads, `files.export` conversions of picked Workspace files, and multipart `files.create`/`files.update` uploads (with byte progress and cancellation) to `www.googleapis.com`/`googleapis.com` | Only when the user attaches a picked Drive file to a draft or saves a message attachment to a picked Drive folder, after the `drive.file` opt-in. Event attachments store only the Drive link — they make no Drive API call | **Off** | Drive attach/save action; revoke the Google grant |
