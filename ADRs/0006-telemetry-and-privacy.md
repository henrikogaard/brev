...
| Google Calendar/Contacts collection listing | Google OAuth access token plus `calendarList` / `contactGroups` read requests to `www.googleapis.com` / `people.googleapis.com` | Only when the user enables the Google calendar/contacts feature or chooses Refresh Collections on the source | **Off** | Settings → Calendar & Contacts source list |
