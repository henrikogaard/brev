...
The Developer ID signing style, certificate, team, and ring-specific
provisioning profile are configured on the `BrevMacOS` app target's Release
configuration. The archive script passes the selected profile through the
custom
`BREV_PROVISIONING_PROFILE_SPECIFIER` build setting rather than setting
`PROVISIONING_PROFILE_SPECIFIER`, certificate, or team globally. SPM package
