...
| Updates | `UpdateSettings`, `UpdateCheckCadence`, `UpdateRing`, `UpdateBuildConfiguration`, `SettingsUpdateActions` | `SettingsPersistenceStore` / `UserDefaults` key `updates.cadence`; ring from `BRReleaseRing` in Info.plist | Capability-gated. Visible for macOS direct-download builds via `SettingsSectionAvailability.macOSDirectDownload`; hidden from `v1Default`. Defaults are once-per-launch checks. The release ring is a build-time property (ADR-0080) shown read-only with a link to the other ring's download; there is no in-app channel switch. Sparkle initialization/manual checks require macOS, direct-download distribution, a feed URL, and a configured public EdDSA key. Update checks contact the GitHub Pages appcast (`henrikogaard.github.io`) only for direct-download macOS builds. |
